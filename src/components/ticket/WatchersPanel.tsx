...
export function WatchersPanel({
  ...
  const {
    ...
    showToast, currentUser, logAuditAction,
  } = useApp();
      ...
      <div className="flex gap-2">
        ...
        <button id="add-watcher-btn" onClick={() => {
          ...
          for (const email of emails) {
            ...
            if ((ticket.watchers || []).map(w => w.toLowerCase()).includes(email.toLowerCase())) { showToast(`${email} already watching.`, 'info'); continue; }
            validEmails.push(email);
          }
          if (validEmails.length === 0) return;
          const updated = tickets.map(t => { if (t.id === ticket.id) { return { ...t, watchers: [...(t.watchers || []), ...validEmails] } } return t; });
          const newNotifications = validEmails.map(email => ({ id: 'wn-' + Date.now() + '-' + Math.random(), timestamp: new Date().toISOString(), ticketId: ticket.id, message: `Added as watcher to ticket ${ticket.id} by ${currentUser.firstName + ' ' + currentUser.lastName}.`, recipient: email, seen: false }));
          ...
          const changedTicket = updated.find(t => t.id === ticket.id);
          ...
          logAuditAction(ticket.id, 'TICKET_WATCHER_ADDED', `Added ${validEmails.join(', ')} to watchers.`);
