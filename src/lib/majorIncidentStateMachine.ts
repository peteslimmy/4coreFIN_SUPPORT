/**
 * Client-side mirror of the Major Incident lifecycle rules. The server is the
 * source of truth; this file keeps the UI in sync so buttons/options disable
 * exactly when the server would reject the change.
 *
 * States: DECLARED → INVESTIGATING → IDENTIFIED → MONITORING → RESOLVED → CLOSED
 */
