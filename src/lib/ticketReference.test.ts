...
describe('Ticket reference tables', () => {
  ...
  describe('GET /ticket/categories', () => {
    it('returns list of categories', async () => {
      ...
      expect(data.some((c: any) => c.code === 'INCIDENT')).toBe(true);
      expect(data.some((c: any) => c.code === 'COMPLAINT')).toBe(true);
    });
  });
  ...
  describe('GET /ticket/severities', () => {
    it('returns list of severities', async () => {
      ...
      const data = await res.json();
      expect(Array.isArray(data)).toBe(true);
      expect(data.some((s: any) => s.code === 'LOW')).toBe(true);
      expect(data.some((s: any) => s.code === 'HIGH')).toBe(true);
    });
  });
