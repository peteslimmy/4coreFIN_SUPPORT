...
import AddReferenceKindModal from '../components/reference/AddReferenceKindModal';
...
export default function ReferenceDataPage() {
  ...
  const handleAddKind = async () => {
    ...
    if (!label) { showToast('Display name is required.', 'error'); return; }
    if (!kind) { showToast('Storage key is required.', 'error'); return; }
    setSavingKind(true);
    try {
      ...
      <PageContainer maxWidth="full">
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <div className="w-full md:w-56 shrink-0">
            <button
              onClick={() => { setDraft(EMPTY_DRAFT); setKindEdited(false); setKindModalOpen(true); }}
        ...
        <AddReferenceKindModal
          open={kindModalOpen}
          onClose={() => setKindModalOpen(false)}
          draft={draft}
          onDraftChange={setDraft}
          kindEdited={kindEdited}
          onKindEdited={() => setKindEdited(true)}
          saving={savingKind}
          onSubmit={handleAddKind}
        />
      </PageContainer>
    </PageTransition>
  );
