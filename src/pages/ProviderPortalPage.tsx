...
export default function ProviderPortalPage() {
  ...
  const providerName = currentUser.bu;
  const providerTickets = useMemo(() =>
    scopedTickets.filter(t =>
      t.provider.toLowerCase() === providerName.toLowerCase() &&
      (t.id.toLowerCase().includes(searchQuery.toLowerCase()) ||
       t.customerName.toLowerCase().includes(searchQuery.toLowerCase()) ||
       t.category.toLowerCase().includes(searchQuery.toLowerCase()))
    ),
    [scopedTickets, providerName, searchQuery]
  );

  const activeInvestigations = providerTickets.filter(t => t.status === TicketStatus.INVESTIGATE).length;
  const pendingRca = providerTickets.filter(t => t.status === TicketStatus.INVESTIGATE && !t.rootCause).length;
  const resolvedCount = providerTickets.filter(t => t.status === TicketStatus.RESOLVED || t.status === TicketStatus.CLOSED).length;
  const avgSatisfaction = (() => {
    const rated = providerTickets.filter(t => t.feedbackScore !== null);
  ...
  const handleCloseTicket = async (ticket: TicketRecord) => {
    // Providers resolve; closing for validation is reserved for BU Support.
    ...
    if (!available.some(r => r.to === TicketStatus.CLOSED)) {
      ...
      showToast(
        `Providers cannot close directly. Available actions from "${ticket.status}": ${allowed || 'none'}. Resolve first; BU Support will close.`,
        'error'
      );
      return;
    }
  ...
  const handleSubmitRca = async (ticket: TicketRecord) => {
    ...
    logAuditAction(ticket.id, 'PROVIDER_SUBMIT_RCA', `Provider ${currentUser.firstName + ' ' + currentUser.lastName} submitted RCA for ticket ${ticket.id}`);
    try {
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <>
            ...
            {providerTickets.length === 0 ? (
              <EmptyState icon={<ClipboardList className="w-12 h-12" />} title="No tickets assigned" message={`No tickets are currently assigned to ${providerName}.`} />
            ) : (
