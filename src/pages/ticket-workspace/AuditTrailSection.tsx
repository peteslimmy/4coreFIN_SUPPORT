...
import type { TicketRecord, AuditLog, CommentRecord } from "../../types/app";
...
/* Ticket-scoped audit actions shown in the lifecycle timeline */
...
/* Static Tailwind class strings so JIT picks them up */
function getEventStyle(action: string): { icon: React.ElementType; dot: string; text: string } {
  ...
  if (a.includes("RCA") || a.includes("AUTO")) {
    return { icon: Bot, dot: "bg-primary-light", text: "text-primary-dark" };
  }
...
/** Human-readable summary for transition events, e.g. "Receipt → Assigned". */
...
export default function AuditTrailSection({ activeTicket }: AuditTrailSectionProps) {
  const { auditLogs, comments, users, showToast } = useApp();
  const [viewMode, setViewMode] = useState<'timeline' | 'detailed'>('timeline');

  /* Unified chronological lifecycle timeline: status transitions, creation,
     escalations, evidence AND every message/communication timestamp. */
  const ticketEvents = useMemo<TimelineEvent[]>(() => {
    if (!activeTicket) return [];

    /* Comments are merged in directly below (richer data) — skip their audit
       duplicates here so each message appears exactly once. */
    const auditEvents: TimelineEvent[] = (auditLogs || [])
      .filter((log: AuditLog) => log.ticketId === activeTicket.id
    ...
    const commentEvents: TimelineEvent[] = (comments || [])
      .filter((c: CommentRecord) => c.ticketId === activeTicket.id)
      .map(c => ({
  ...
  }, [activeTicket, auditLogs, comments]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-border">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={`px-2 py-1 rounded text-[9px] font-medium transition-colors focus-ring ${viewMode === 'timeline' ? 'bg-primary/10 text-primary' : 'text-text-muted hover:bg-surface-hover'}`}
          >
            Timeline
          </button>
          <button
            ...
            className={`px-2 py-1 rounded text-[9px] font-medium transition-colors focus-ring ${viewMode === 'detailed' ? 'bg-primary/10 text-primary' : 'text-text-muted hover:bg-surface-hover'}`}
          >
            Detailed
          </button>
        </div>
              ...
              <div key={ev.id} className="py-2.5 first:pt-0 last:pb-0">
                <div className="flex items-center justify-between gap-2 mb-1">
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className="font-mono text-[10px] font-semibold text-text-primary truncate">
                      {ev.action}
                    </span>
                  </div>
                  <span className="text-[9px] text-text-muted shrink-0" title={new Date(ev.timestamp).toLocaleString()}>
                ...
                <dl className="ml-5 grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 text-[9px]">
                  ...
                  <dd className="text-text-secondary break-words whitespace-pre-wrap">
                    {eventSummary(ev)}
                  </dd>
                </dl>
              </div>
      ...
      <div className="flex items-center justify-between pt-1">
        <span className="text-[9px] text-text-muted flex items-center gap-1">
          <Zap className="w-3 h-3" /> Immutable audit record
        </span>
        <button
          onClick={() => showToast('Audit trail refreshed.', 'success')}
          className="text-[9px] font-medium text-primary hover:text-primary-dark hover:underline focus-ring"
        >
