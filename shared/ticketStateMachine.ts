/**
 * Canonical ticket state machine (moved from src/lib — server previously
 * imported frontend code directly across the app/server boundary).
 */
import type { TicketRecord } from '../src/types/app';
...
const WAITING_TO_ACTIVE_STEP: Partial<Record<TicketStatus, TicketStatus>> = {
  ...
  [TicketStatus.REOPENED]: TicketStatus.INVESTIGATE,
};
...
export const TRANSITIONS: TransitionRule[] = [
  ...
  // deadline shifts forward without rewriting the original.
  {
    ...
    roles: INVESTIGATOR_ROLES,
    canTransition: () => true,
    mutate: (t) => ({ slaPauseStartedAt: new Date().toISOString() }),
  },
  {
    from: TicketStatus.INVESTIGATE,
  ...
  },
  {
    ...
    event: 'REOPEN',
    label: 'Reopen',
    customerFacingLabel: 'Reopened',
    roles: BU_AGENT_ROLES,
    required: [
      (t) => !!t.reopenJustification && t.reopenJustification.trim() !== '',
    ],
    requiredLabels: ['Reopen Justification'],
    mutate: (t, ctx) => ({
      reopenJustification: t.reopenJustification,
      reopenReason: t.reopenReason,
      reopenAt: new Date().toISOString(),
      reopenBy: ctx?.actor || t.assignedAgentId,
      isEscalated: true,
  ...
  },
  {
    ...
    to: TicketStatus.ASSIGNED,
    event: 'ASSIGN_REOPENED',
    label: 'Assign Reopened Ticket',
    customerFacingLabel: 'Assigned',
    roles: BU_AGENT_ROLES,
    canTransition: (t) => !!t.assignedAgentId,
    mutate: (t) => ({ status: TicketStatus.ASSIGNED }), // auto-advance or explicit
  },
...
export function applyTransition<T extends TicketRecord>(
  ...
  // Accumulate any in-progress SLA pause BEFORE applying the transition's mutate.
  // This prevents a race where a rapid WAIT -> RESUME -> WAIT cycle would lose
  // the first pause span because the second WAIT sets a new slaPauseStartedAt
  // before accumulatePause() runs.
  let pauseAccumulated: Partial<TicketRecord> = {};
  ...
  if (waitingStatuses.includes(ticket.status)) {
    ...
    if (startedAt) {
      const span = Math.max(0, Date.now() - startedAt);
      pauseAccumulated = { slaPausedMs: (ticket.slaPausedMs || 0) + span, slaPauseStartedAt: null };
    }
  }
