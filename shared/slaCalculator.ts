// Canonical SLA calculator (moved from src/lib — server previously imported
// frontend code directly across the app/server boundary).
import { SlaRule, HolidayRecord } from '../src/types/admin';
import { TicketPriority, TicketStatus, type TicketRecord } from '../src/types/app';
...
export interface SlaDeadlineInfo {
  ...
  rule?: SlaRule;
  /** Whether the SLA rule was partner-specific (true) or category/default (false) */
 ...
 * Implements partner-specific → category-specific → default hierarchy.
 */
export function resolveSlaDuration(
  ...
  slaRules: SlaRule[],
  partnerOrgId?: string
): { durationHours: number; source: SlaSource; rule?: SlaRule; partnerSpecific: boolean } {
  // 1. Try partner-specific rule first (if partnerOrgId provided)
  if (partnerOrgId) {
    const partnerMatchingRule = slaRules.find(
      rule => 
        rule.partnerOrgId === partnerOrgId &&
        rule.category.toLowerCase() === category.toLowerCase() && 
        rule.priority.toLowerCase() === priority.toLowerCase()
    );
    
    if (partnerMatchingRule) {
      return { 
        durationHours: partnerMatchingRule.durationHours, 
        source: 'rule', 
        rule: partnerMatchingRule,
        partnerSpecific: true 
      };
    }
  }
  
  // 2. Fall back to category-specific default rule (no partnerOrgId)
  const categoryMatchingRule = slaRules.find(
    rule => 
      !rule.partnerOrgId && // No partner-specific (default rule)
      rule.category.toLowerCase() === category.toLowerCase() && 
      rule.priority.toLowerCase() === priority.toLowerCase()
  );
  
  if (categoryMatchingRule) {
    return { 
      durationHours: categoryMatchingRule.durationHours, 
      source: 'rule', 
      rule: categoryMatchingRule,
      partnerSpecific: false 
    };
  }
  
  // 3. Ultimate fallback to priority-based default
  ...
  return { durationHours, source: 'fallback', partnerSpecific: false };
}
...
export function computeSlaDeadline(
  ...
  const { durationHours, source, rule, partnerSpecific } = resolveSlaDuration(category, priority, slaRules, partnerOrgId);
  ...
  if (businessHoursConfig) {
    ...
    return { deadline, source, durationHours, rule, partnerSpecific };
  }
...
export function calculateSlaDeadline(
  ...
  return computeSlaDeadline(createdAt, category, priority, slaRules, holidays, businessHoursConfig, partnerOrgId).deadline;
}